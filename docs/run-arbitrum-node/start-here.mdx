...
- Nitro stores its state trie using either <a data-quicklook-from="hashdb">HashDB</a> (the default) or <a data-quicklook-from="pathdb">PathDB</a>. The choice affects which snapshot you can use, so make it before you initialize the database. Both <a data-quicklook-from="state-scheme">schemes</a> have published full node snapshots; PathDB <a data-quicklook-from="state-pruning">prunes</a> automatically but cannot validate blocks. See [Choose a state scheme](/run-arbitrum-node/02-run-full-node.mdx#choose-a-state-scheme).
...
Latest [Docker image](https://hub.docker.com/r/offchainlabs/nitro-node/tags): <code>@@latestNitroNodeImage=offchainlabs/nitro-node:v3.11.3-4130f4c@@</code>
...
If you're running a beacon node, historical data will now be in <a data-quicklook-from="blob">blobs</a>. To make this transition to using historical blobs, refer to the [Historical Blobs for Beacon Nodes](/run-arbitrum-node/beacon-nodes-historical-blobs.mdx) guide.
