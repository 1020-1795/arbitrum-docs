...
For background on how sequencers fit into Arbitrum's data flow (batch posting, the sequencer feed, and how full nodes consume sequencer output), see [Data availability](/run-arbitrum-node/data-availability.mdx). For how the sequencer role relates to the other Nitro node roles and the flags that define them, see [How to assign roles to a Nitro node](/run-arbitrum-node/assign-node-roles.mdx).
...
Latest Docker image: <code>@@latestNitroNodeImage=offchainlabs/nitro-node:v3.11.3-4130f4c@@</code>
...
As of Nitro v3.10.0, these settings live under `--node.da.anytrust.*` (the previous `--node.data-availability.*` namespace is deprecated), and the node no longer takes `sequencer-inbox-address` or `parent-chain-node-url` in this section — it uses the chain info and the `--parent-chain.connection.url` configuration instead.

:::

Enable your node to send batches to a <a data-quicklook-from="data-availability-server-das">DAS</a> and get DACerts from them.

```shell
--node.da.anytrust.enable=true
--node.da.anytrust.max-batch-size=90000
--node.da.anytrust.rest-aggregator.enable=true
--node.da.anytrust.rest-aggregator.urls=<A list of DAS REST endpoints, can be only one URL>
--node.da.anytrust.rpc-aggregator.enable=true
--node.da.anytrust.rpc-aggregator.assumed-honest=1
--node.da.anytrust.rpc-aggregator.backends=<A list of RPC backends>
```
