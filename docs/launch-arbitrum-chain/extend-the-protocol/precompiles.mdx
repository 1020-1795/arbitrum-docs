...
To support these additional use cases, follow the instructions described in [How to customize your Arbitrum chain's behavior](./stf.mdx).

:::

There are five primary ways to customize your chain's precompiles:

1.  Add new methods to an existing [precompile](https://github.com/OffchainLabs/@@nitroRepositorySlug=nitro@@/blob/@@nitroVersionTag=v3.11.3@@/@@nitroPathToPrecompiles=precompiles@@/) (local: `/precompiles`)
...
```shell
git clone --branch @@nitroVersionTag=v3.11.3@@ <https://github.com/OffchainLabs/nitro.git>
cd nitro
...
Next, follow the steps in [How to customize your Arbitrum chain's behavior](./stf.mdx#step-3-run-the-node) to build a modified Arbitrum Nitro node Docker image and run it.
...
Then, update [precompile.go](https://github.com/OffchainLabs/@@nitroRepositorySlug=nitro@@/blob/@@nitroVersionTag=v3.11.3@@/@@nitroPathToPrecompiles=precompiles@@/precompile.go) to register the new precompile under the `Precompiles()` method:
...
To learn more about the gas cost model, see [how to estimate gas](/arbitrum-essentials/how-to-estimate-gas.mdx).
...
First, open the [arbosstate.go](https://github.com/OffchainLabs/@@nitroRepositorySlug=nitro@@/blob/@@nitroVersionTag=v3.11.3@@/@@nitroPathToArbosState=arbos/arbosState@@/arbosstate.go) file and locate the [ArbosState](https://github.com/OffchainLabs/nitro/blob/v2.2.0/arbos/arbosState/arbosstate.go#L38) structure. This is where ArbOS state is defined.

Define a state key called `myNumber` of type `storage.StorageBackedUint64`. You can find more types in [storage.go](https://github.com/OffchainLabs/@@nitroRepositorySlug=nitro@@/blob/@@nitroVersionTag=v3.11.3@@/@@nitroPathToStorage=arbos/storage@@/storage.go):
...
Next, head back to the [precompiles directory](https://github.com/OffchainLabs/@@nitroRepositorySlug=nitro@@/blob/@@nitroVersionTag=v3.11.3@@/@@nitroPathToPrecompiles=precompiles@@/) and create a new `ArbHi.go` (introduced in Option 2).
...
If you do not customize the precompile before launching your Arbitrum chain network, please continue to follow the instructions on [Customize ArbOS](./arbos.mdx) to perform an ArbOS version control to avoid blockchain reorg.
