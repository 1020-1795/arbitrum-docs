...
1. **An RPC interface** that the [Sequencer](/how-arbitrum-works/deep-dives/sequencer.mdx) uses to store batches of data on the DAS.
2. **An HTTP REST interface** that lets the DAS respond to requests for those batches of data.

For more information related to configuring a DAC, refer to the [Introduction](/launch-arbitrum-chain/chain-config/data-availability/dac-get-started.mdx).
...
A DAS can be configured to use one or more of three storage backends:
...
<VanillaAdmonition type="warning" title="Binary and flag names changed in Nitro v3.10.0">

As of Nitro v3.10.0, the `daserver` and `datool` binaries have been renamed to `anytrustserver` and `anytrusttool`, and the parent chain flags moved: `--data-availability.parent-chain-node-url` is now `--parent-chain.node-url`, and `--data-availability.sequencer-inbox-address` is now `--parent-chain.sequencer-inbox-address`. Current Docker images (v3.11.2 and later) only ship the new binaries, so scripts using the old names fail. This guide uses the new names throughout.
...
- The latest Nitro docker image: `@@latestNitroNodeImage=offchainlabs/nitro-node:v3.11.3-4130f4c@@`
...
The BLS keypair must be generated using the `anytrusttool keygen` utility. Later, it will be passed to the DAS by file or command line.
...
Here's an example of how to use the `anytrusttool keygen` utility inside Docker and store the key that will be used by the DAS in the next step.

```shell
docker run -v $(pwd)/bls_keys:/data/keys --entrypoint anytrusttool \
@@latestNitroNodeImage=offchainlabs/nitro-node:v3.11.3-4130f4c@@ keygen --dir /data/keys
```

### Step 2: Deploy the DAS

To run the DAS, we'll use the `anytrustserver` tool and we'll configure the following parameters:

| Parameter                              | Description                                                                                                     |
| -------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| --parent-chain.node-url                | RPC endpoint of a parent chain node                                                                             |
| --parent-chain.sequencer-inbox-address | Address of the `SequencerInbox` in the parent chain                                                             |
...
| --log-level                            | Log level: CRIT, ERROR, WARN, INFO, DEBUG, or TRACE (default INFO)                                              |
...
import S3Parameters from '../../partials/config-snippets/_s3-parameters.mdx';
import LocalFilesParameters from '../../partials/config-snippets/_local-files-parameters.mdx';
import GoogleCloudStorageParameters from '../../partials/config-snippets/_gcs-parameters.mdx';
...
Here's an example `anytrustserver` command for a DAS that:
...
First we'll generate an ECDSA keypair with `anytrusttool keygen`. Create a folder inside `/some/local/dir` to store the ECDSA keypair, for example `/some/local/dir/keys`. Then run `anytrusttool keygen`:

```shell
anytrusttool keygen --dir /some/local/dir/keys --ecdsa
```

You can also use the `docker run` command as follows:

```shell
docker run --rm -it -v /some/local/dir:/home/user/data --entrypoint anytrusttool @@latestNitroNodeImage=offchainlabs/nitro-node:v3.11.3-4130f4c@@ keygen --dir /home/user/data/keys --ecdsa
```

#### Step 2: Change the DAS configuration and restart the server

Add the following configuration parameter to `anytrustserver`:
...
Now you can use the `anytrusttool` utility to send store requests signed with the ECDSA private key:

```shell
anytrusttool client rpc store --rpc-client.rpc.url http://localhost:9876 --message "Hello world" --signing-key /some/local/dir/keys/ecdsa
```

OR

```shell
anytrusttool client rpc store --rpc-client.rpc.url http://localhost:9876 --message "Hello world" --signing-key "0x<contents of ecdsa>"
```

You can also use the `docker run` command:

```shell
docker run --rm -it -v /some/local/dir:/home/user/data --network="host" --entrypoint anytrusttool @@latestNitroNodeImage=offchainlabs/nitro-node:v3.11.3-4130f4c@@ client rpc store --rpc-client.rpc.url http://localhost:9876 --message "Hello world" --signing-key /home/user/data/keys/ecdsa
```
...
Use again the `anytrusttool` to retrieve the stored data. Notice that to perform this step you must have the REST interface enabled in the DAS:

```shell
anytrusttool client rest getbyhash --url http://localhost:9877 --data-hash 0xDataHash
```

You can also use the `docker run` command:

```shell
docker run --rm -it --network="host" --entrypoint anytrusttool @@latestNitroNodeImage=offchainlabs/nitro-node:v3.11.3-4130f4c@@ client rest getbyhash --url http://localhost:9877 --data-hash 0xDataHash
```
...
The retention period defaults to 24 hours, but can be configured when calling `anytrusttool client rpc store` with the parameter `--anytrust-retention-period` and a duration for the retention period (e.g., `48h`).
...
Find information about how to set up a mirror DAS in [How to deploy a mirror DAS](/launch-arbitrum-chain/chain-config/data-availability/deploy-mirror-das.mdx).
